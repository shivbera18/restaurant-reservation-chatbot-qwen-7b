...
export const RestaurantCard: React.FC<RestaurantCardProps> = ({
  ...
  return (
    <div className="bg-white border-2 border-black rounded-neo shadow-neo p-3.5 flex flex-col justify-between text-left transition-all">
      <div>
        {/* Top bar: Name & Price */}
        <div className="flex items-start justify-between gap-2">
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-0.5 text-xs text-black font-bold">
              <MapPin className="w-3.5 h-3.5 text-neo-orange shrink-0" />
              <span>{neighborhoodVal}</span>
              {restaurant.address && (
                <>
                  <span>•</span>
                  <span className="text-black font-mono text-[11px] truncate max-w-[160px]">
                    {restaurant.address}
                  </span>
                </>
              )}
          ...
          <div className="flex flex-col items-end gap-1 shrink-0">
            <span className="bg-neo-yellow text-black border border-black font-mono font-black text-xs px-2 py-0.5 shadow-neo-sm rounded-neo-sm">
              {priceVal}
            </span>
            <div className="flex items-center gap-0.5 bg-black text-white px-1.5 py-0.5 text-[11px] font-mono font-bold rounded-neo-sm border border-black">
        ...
        {(cuisines.length > 0 || ambiances.length > 0) && (
          <div className="flex flex-wrap items-center gap-1.5 mt-2.5">
            {cuisines.map((cuisine) => (
              <span
                key={cuisine}
                className="bg-neo-blue/20 text-black border border-black px-1.5 py-0.5 text-[10px] font-mono uppercase font-black rounded-neo-sm"
              >
                🍽️ {cuisine}
              </span>
            ))}
            {ambiances.slice(0, 2).map((amb) => (
              <span
                key={amb}
                className="bg-neo-purple/20 text-black border border-black px-1.5 py-0.5 text-[10px] font-mono uppercase font-black rounded-neo-sm"
              >
                🎭 {amb}
              </span>
            ))}
        ...
        {restaurant.description && !compact && (
          <p className="text-xs text-black mt-2 line-clamp-2 leading-relaxed font-bold">
        ...
        {popularDishes.length > 0 && !compact && (
          <div className="mt-2.5 pt-2 border-t border-dashed border-black">
            <div className="text-[10px] font-mono uppercase font-black text-black flex items-center gap-1">
            ...
            <div className="flex flex-wrap gap-1 mt-1">
              {popularDishes.map((dish) => (
                <span
                  key={dish}
                  className="bg-neo-canvas border border-black text-black text-[10px] font-sans px-1.5 py-0.5 rounded-neo-sm font-bold"
                >
                  {dish}
                </span>
              ))}
        ...
        {/* Features / Amenities */}
        <div className="flex flex-wrap items-center gap-2 mt-2.5 text-[10px] font-mono text-black font-bold">
